...
static ge::graphStatus TilingTreeGDN(gert::TilingContext *context)
{
    ...
    if (!ShapeIs(context->GetInputShape(0), {n, hk, KEY_DIM}) ||
        ...
        !ShapeIs(context->GetOutputShape(1), {n, hv, VALUE_DIM, KEY_DIM})) {
    ...
    for (uint32_t i = 0; i < 2; ++i) {
        ...
        if (desc == nullptr || desc->GetStorageFormat() != ge::FORMAT_ND ||
            desc->GetDataType() != ge::DT_FLOAT16) {
            return ge::GRAPH_FAILED;
        }
    }
    ...
    // Conservative initial candidate from A1. R64 is explicit until measured tuning.
    bool fits = false;
    const uint32_t candidates[] = {32, 16};
...
IMPL_OP_OPTILING(TreeGatedDeltaRuleV310)
