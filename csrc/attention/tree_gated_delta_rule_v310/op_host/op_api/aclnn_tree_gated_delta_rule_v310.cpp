...
#include "aclnn_tree_gated_delta_rule_v310.h"
...
using namespace op;
namespace l0op {
OP_TYPE_REGISTER(TreeGatedDeltaRuleV310);
...
static aclnnStatus Launch(const aclTensor *query, const aclTensor *key,
    ...
    aclTensor *out, aclTensor *snapshots, aclOpExecutor *executor)
{
    // Direct outputs: no intermediate full-state buffer and no ViewCopy of snapshots.
    return ADD_TO_LAUNCHER_LIST_AICORE(TreeGatedDeltaRuleV310,
        OP_INPUT(query, key, value, beta, initialState, g),
        OP_OUTPUT(out, snapshots), OP_ATTR(parents, scale, vTile));
}
} // namespace l0op
...
extern "C" aclnnStatus aclnnTreeGatedDeltaRuleV310GetWorkspaceSize(
    ...
    aclTensor *snapshots, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ...
    const aclTensor *tensors[] = {query, key, value, beta, initialState, g, out, snapshots};
    for (size_t i = 0; i < 8; ++i) {
        const auto *tensor = tensors[i];
        if (tensor == nullptr || tensor->GetData() == nullptr || !IsContiguous(tensor) ||
            tensor->GetStorageFormat() != Format::FORMAT_ND ||
            tensor->GetDataType() != (i == 5 ? DataType::DT_FLOAT : DataType::DT_FLOAT16)) {
            return ACLNN_ERR_PARAM_INVALID;
        }
    }
    ...
    if (n < 1 || n > TreeGDN310::MAX_NODES || hk < 1 || hv < hk || hv > 32 || hv % hk ||
        ...
        !ShapeIs(out, {n, hv, 128}) || !ShapeIs(snapshots, {n, hv, 128, 128})) {
        return ACLNN_ERR_PARAM_INVALID;
    }
    ...
    for (size_t i = 0; i < 8; ++i) {
        ...
        const uint64_t bytes = static_cast<uint64_t>(tensors[i]->Numel()) * (i == 5 ? 4U : 2U);
    ...
    L2_DFX_PHASE_1(aclnnTreeGatedDeltaRuleV310,
        DFX_IN(query, key, value, beta, initialState, g, parents, scale, vTile),
        DFX_OUT(out, snapshots));
    ...
    const auto status = l0op::Launch(query, key, value, beta, initialState, g, parents,
                                     scale, vTile, out, snapshots, uniqueExecutor.get());
...
extern "C" aclnnStatus aclnnTreeGatedDeltaRuleV310(
    void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnTreeGatedDeltaRuleV310);
