...
public:
    __aicore__ inline void Init(
        ...
        GM_ADDR g, GM_ADDR out, GM_ADDR snapshots,
        const TreeGatedDeltaRuleV310TilingData *td, TPipe *pipe)
    {
        ...
        snapshotsGm_.SetGlobalBuffer(reinterpret_cast<__gm__ half *>(snapshots));
...
private:
    ...
    __aicore__ inline void ProcessTile(uint32_t head, uint32_t firstRow)
    {
        ...
        for (uint32_t step = 0; step < td_->nodes; ++step) {
            ...
            Muls(decayed, state, gamma.GetValue(gate), m);
            PipeBarrier<PIPE_V>();
            ...
            DataCopy(snapshotsGm_[vOffset * KEY_DIM], stack[(depth + 1) * m], m);
    ...
    GlobalTensor<half> outGm_, snapshotsGm_;
